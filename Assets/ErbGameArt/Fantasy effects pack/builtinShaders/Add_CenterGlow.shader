// Hand-converted to URP (Universal Render Pipeline) from the original Amplify/Built-in shader.
// Functionally identical: same properties, same blend (additive One One), same per-pixel math,
// same soft-depth fade (_Usedepth) and fog.
	...
	SubShader
	{
		...
		Pass
		{
			...
			Varyings vert (Attributes v)
			{
				...
				o.projPos.z = -vp.positionVS.z;            // eye-space depth (positive)
				o.color = v.color;
			...
			half4 frag (Varyings i) : SV_Target
			{
				...
				// Soft-particle depth fade — gated by _Usedepth (matches the original toggle).
				...
				half4 col = ( lerp(temp_output_30_0,( temp_output_30_0 * clampResult40 ),_Usecenterglow) * _Emission );
				col.rgb = MixFogColor(col.rgb, half3(0,0,0), i.fogCoord);   // additive → fade to black (matches original)
				return col;
			}
			ENDHLSL
		}
