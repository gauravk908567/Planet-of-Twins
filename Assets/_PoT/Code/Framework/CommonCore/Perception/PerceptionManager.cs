...
namespace CommonCore
{
    ...
    public static class PerceptionManagerBootstrapper
    {
        ...
        {
            // Multi-scene: AfterSceneLoad fires when Bootstrap (scene 0) loads — Persistent and its
            // PerceptionManager are NOT loaded yet. Calling PerceptionManager.Instance here would
            // FABRICATE a blank "Singleton<PerceptionManager>" GameObject (see MonoBehaviourSingleton
            // line 41), which then wins the singleton and the real Persistent manager destroys itself
            // on Awake — leaving every listener/perceivable registered against a dead manager.
            // Use FindFirstObjectByType (never fabricates); if the real manager isn't present yet,
            // do nothing — OnBootstrapped is a no-op for PerceptionManager anyway.
            var existing = UnityEngine.Object.FindFirstObjectByType<PerceptionManager>();
            if (existing != null)
                existing.OnBootstrapped();
        }
    }
}
