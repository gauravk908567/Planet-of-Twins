...
public abstract class UtilityGOAPGoalBase : GOAPGoalBase
{
    ...
    private float ReadBlackboardValue(UtilityFactor factor)
    {
        ...
        {
            bool val = false;
            if (LinkedBlackboard.TryGet(key, out val, false))
                return val ? 1f : 0f;

            // Fallback: "presence" keys such as Awareness.BestTarget.GameObject store a GameObject
            // reference, not a bool (the package's own goals check this slot for != null — there is
            // no bool target flag). The bool dictionary read above misses it because values are
            // stored per-type. Treat "reference is set" as true. (Matches package GOAPGoal_Chase.)
            GameObject go = null;
            if (LinkedBlackboard.TryGet(key, out go, null))
                return go != null ? 1f : 0f;
