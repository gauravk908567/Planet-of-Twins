...
public class POIManager : MonoBehaviour
{
    ...
    /// <summary>All active POIs of a type whose position is inside <paramref name="bounds"/> (a zone's
    /// collider bounds), into <paramref name="into"/> (cleared first). Lets a SpawnZone scope its POI
    /// logic to itself with no hand-placed serialized array — POIs self-register here on OnEnable.</summary>
    public void GetAllInBounds(Bounds bounds, POIType type, List<POIBase> into, bool activeOnly = true)
    {
        into.Clear();
        foreach (var poi in _pois)
        {
            if (poi == null || poi.PoiType != type) continue;
            if (activeOnly && !poi.IsActive) continue;
            if (bounds.Contains(poi.transform.position)) into.Add(poi);
        }
    }
