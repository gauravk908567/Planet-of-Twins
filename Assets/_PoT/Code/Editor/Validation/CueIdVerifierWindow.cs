...
namespace PlanetOfTwins.EditorTools
{
    ...
    public class CueIdVerifierWindow : EditorWindow
    {
        ...
        // Headless run of the same scan (no window) so a batch / pipeline step can check cue ids like Path Health.
        // One log line per finding: the console reader keeps only a log's first line.
        [MenuItem("Planet of Twins Tools/Validation/Cue Ids")]
        public static void ValidateToLog()
        {
            int errors = 0, warnings = 0, infos = 0;
            foreach (var f in Scan())
            {
                string where = string.IsNullOrEmpty(f.file) ? "" : $" [{f.file}:{f.line}]";
                if (f.type == MessageType.Error) { errors++; Debug.LogError($"[CueIdVerifier] {f.msg}{where}", f.ping); }
                else if (f.type == MessageType.Warning) { warnings++; Debug.LogWarning($"[CueIdVerifier] {f.msg}{where}", f.ping); }
                else infos++;
            }
            Debug.Log($"[CueIdVerifier] {errors} error(s), {warnings} warning(s), {infos} info(s)." +
                      (errors == 0 ? " Every PlayBook id resolves to a Cue Book effect." : " Fix the errors above."));
        }

        [MenuItem("Planet of Twins Tools/Validation/Generate FxIds")]
        public static void GenerateFxIdsMenu() => GenerateFxIds();
        ...
        private void Validate()
        {
            ...
            foreach (var f in _findings)
            {
                if (f.type == MessageType.Error) _errors++;
                else if (f.type == MessageType.Warning) _warnings++;
            }
            Repaint();
        }

        // The whole check, shared by the window and the headless menu item.
        private static List<Finding> Scan()
        {
            var found = new List<Finding>();
            void Add(MessageType t, string msg, Object ping, string file = null, int line = 0)
                => found.Add(new Finding { type = t, msg = msg, ping = ping, file = file, line = line });
            ...
            if (Directory.Exists(ScriptRoot))
            {
                foreach (var file in Directory.GetFiles(ScriptRoot, "*.cs", SearchOption.AllDirectories))
                {
                    ...
                    {
                        string line = lines[i];
                        if (line.TrimStart().StartsWith("//")) continue;   // doc/comment examples aren't call sites
 ...
 // so nothing is lost. (+ the VFX Library layer, 2026-06-22.)
        private static void GenerateFxIds()
        {
