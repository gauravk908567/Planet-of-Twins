...
public class TutorialCheckpointStepSO : TutorialStepBase
{
 ...
 $"the multi-scene R2 break, ). The checkpoint will never appear until fixed.");
        ...
        try
        {
            // Clear any stale IsCompleted from a prior play/soft-reset before activating.
            // IsCompleted is only ever reset by FullReset (wrong-twin path) — without this, a
            // checkpoint completed in a previous run stays completed, so Activate() no-ops and
            // the marker never appears ("rescue checkpoint not active"). Cold start works, warm
            // restart / soft-reset does not (domain-reload-dependent). See BUG-021/BUG-043.
            cp.FullReset();
            cp.Activate();
    ...
    private IEnumerator RunDual(TutorialStepContext ctx)
    {
        ...
        try
        {
            // Clear stale IsCompleted from a prior play/soft-reset before activating (see RunSingle / BUG-043).
            cpA.FullReset();
            cpB.FullReset();
            cpA.Activate();
            cpB.Activate();
