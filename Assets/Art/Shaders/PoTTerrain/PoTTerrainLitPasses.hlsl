// PoT/TerrainLit passes — port of URP TerrainLitPasses.hlsl (URP 17.3, 2026-07-16).
// PoT additions, each behind a runtime-toggleable keyword (multi_compile — options menu):
//   _POT_HEX      hex-grid tile break-up on the albedo splats (3 rotated samples, blended)
//   _POT_PARALLAX cheap one-tap parallax from mask-map height (terrain approximated y-up)
//   corruption    always compiled; no-op while the _WorldCorruption global is 0
// Everything else is stock — do not drift from the package file except inside PoT blocks.
#ifndef POT_TERRAIN_LIT_PASSES_INCLUDED
#define POT_TERRAIN_LIT_PASSES_INCLUDED

#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/GBufferOutput.hlsl"
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/DBuffer.hlsl"

// ─── PoT uniforms (own cbuffer — terrain material is per-terrain, SRP-batcher n/a) ───
CBUFFER_START(PoTTerrain)
    float  _PoTHexScale;          // hex grid density in splat-UV space (default 0.12)
    float  _PoTHexContrast;       // blend sharpening between the 3 hex samples (default 6)
    float  _PoTParallaxStrength;  // one-tap parallax offset strength (default 0.02)
    half4  _PoTCorruptionColor;   // Voreth family — keep in sync with the Coexistence set
    float  _PoTCorruptionStrength;// max tint at _WorldCorruption = 1 (default 0.85)
    float  _PoTCorrNoiseScale;    // world-space noise scale for the feathered front
    float  _PoTCorrNoiseAmount;   // 0 = uniform tint, 1 = fully noise-gated front
CBUFFER_END
float _WorldCorruption;           // global — WorldAmbienceDriver owns it
float _PoTWetness;                // global — RainController owns it (0 dry .. 1 soaked)

float2 PoTHash2(float2 p)
{
    return frac(sin(mul(float2x2(127.1, 311.7, 269.5, 183.3), p)) * 43758.5453);
}

float PoTNoise2(float2 x)
{
...
// Hex-grid tile break-up (Mikkelsen-style): pick the 3 surrounding triangle-grid vertices,
// sample the texture with a random per-vertex UV offset, blend with sharpened weights.
// Gradients come from the ORIGINAL uv so mip selection stays seam-free.
half4 PoTSampleSplat(TEXTURE2D_PARAM(tex, samp), float2 uv)
{
#if defined(_POT_HEX)
    float2 skewUV = mul(float2x2(1.0, 0.0, -0.57735027, 1.15470054), uv * _PoTHexScale);
    ...
    if (bary.z > 0.0)
    {
        ...
        v1 = baseCell; v2 = baseCell + float2(0, 1); v3 = baseCell + float2(1, 0);
    }
    else
    {
        w = float3(-bary.z, 1.0 - bary.y, 1.0 - bary.x);
        v1 = baseCell + 1.0; v2 = baseCell + float2(1, 0); v3 = baseCell + float2(0, 1);
    }
    float2 dx = ddx(uv), dy = ddy(uv);
    half4 c1 = SAMPLE_TEXTURE2D_GRAD(tex, samp, uv + PoTHash2(v1), dx, dy);
    half4 c2 = SAMPLE_TEXTURE2D_GRAD(tex, samp, uv + PoTHash2(v2), dx, dy);
    half4 c3 = SAMPLE_TEXTURE2D_GRAD(tex, samp, uv + PoTHash2(v3), dx, dy);
    w = pow(max(w, 1e-4), _PoTHexContrast);
    w /= (w.x + w.y + w.z);
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    // PoT: hex tile break-up (falls back to a single stock sample when _POT_HEX is off)
    diffAlbedo[0] = PoTSampleSplat(TEXTURE2D_ARGS(_Splat0, sampler_Splat0), uvSplat01.xy);
    diffAlbedo[1] = PoTSampleSplat(TEXTURE2D_ARGS(_Splat1, sampler_Splat0), uvSplat01.zw);
    diffAlbedo[2] = PoTSampleSplat(TEXTURE2D_ARGS(_Splat2, sampler_Splat0), uvSplat23.xy);
    diffAlbedo[3] = PoTSampleSplat(TEXTURE2D_ARGS(_Splat3, sampler_Splat0), uvSplat23.zw);
...
#endif
{
...
#if defined(_POT_PARALLAX)
    // PoT: cheap one-tap parallax — mask-map height (B), terrain approximated y-up so the
    // world-space view direction xz doubles as the tangent-space offset direction.
    {
        half3 pView = GetWorldSpaceNormalizeViewDir(IN.positionWS);
        float2 pDir = pView.xz / max(pView.y, 0.25) * _PoTParallaxStrength;
        IN.uvSplat01.xy += pDir * (masks[0].b - 0.5h);
        IN.uvSplat01.zw += pDir * (masks[1].b - 0.5h);
        IN.uvSplat23.xy += pDir * (masks[2].b - 0.5h);
        IN.uvSplat23.zw += pDir * (masks[3].b - 0.5h);
    }
#endif
    ...
    // PoT: world corruption — a COLOUR FILM over the ground (blood-moon model, user canon):
    // multiplicative tint so every texture detail survives underneath; the film's colour cast
    // grows with the same _WorldCorruption global everything else reads, with a feathered
    // noisy front. Exact no-op while the global is 0.
    {
        half corr = saturate(_WorldCorruption) * _PoTCorruptionStrength;
        if (corr > 0.001h)
        {
            half n = PoTNoise2(IN.positionWS.xz * _PoTCorrNoiseScale);
            half front = saturate(corr * lerp(1.0h, n * 1.6h, _PoTCorrNoiseAmount));
            // x2 multiply keeps mid-tones near their original brightness under the cast
            half3 film = albedo * _PoTCorruptionColor.rgb * 2.0h;
            albedo = lerp(albedo, film, front);
        }
    }

    // PoT: rain wetness — global film from RainController. Wet ground darkens and turns
    // glossy (PBR path: darken albedo + push smoothness toward a wet ceiling). No-op at 0.
    {
        half wet = saturate(_PoTWetness);
        albedo *= lerp(1.0h, 0.62h, wet);
        smoothness = lerp(smoothness, max(smoothness, 0.78h), wet);
    }
