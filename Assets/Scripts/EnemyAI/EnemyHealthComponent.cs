...
public class EnemyHealthComponent : MonoBehaviour, IDamageable
{
    ...
    /// <summary>Fires on every damage hit with the FULL <see cref="DamageData"/> (carries the attacker via
 /// <c>Source</c>). Routed by <c>EnemyDeathNotifier.OnEnemyDamaged</c> for reaction glyphs.</summary>
    public event Action<EnemyHealthComponent, DamageData> OnDamaged;
 ...
 OnDamaged?.Invoke(this, data); // full payload (attacker via Source) — reactions
    ...
    /// <summary>Pool reset (called from Enemy.ResetForPool). Health was only initialised in Awake,
    /// so a pooled reuse of a killed enemy spawned with 0 HP: IsDead=true made TakeDamage a no-op —
    /// an unkillable standing corpse that never returned to the pool (BUG-058).</summary>
    public void ResetToFull()
    {
        _currentHealth = maxHealth;
        LastDamageType = DamageType.Environmental;
        OnHealthChanged?.Invoke(1f);
    }

    public void ClearDeathSubscribers() { OnDeath = null; OnDamaged = null; }
}