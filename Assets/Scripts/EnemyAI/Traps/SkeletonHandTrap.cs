...
public class SkeletonTrap : MonoBehaviour, IRescueTarget, IDamageable, IStunnable, IPossessable
{
    ...
    private void OnEnable()
    {
        ...
        if (SoftResetController.Instance != null)
            SoftResetController.Instance.OnSoftReset += ForceReset;
    }
    ...
    private void SetMaterial(TrapState state)
    {
        ...
        MaterialTint.SetColor(trapRenderer.material, c);
    }
    ...
    {
        // Real-time grace (see PlayerDeathRescueProxy.InvincibilityFrames / BUG-044): unscaled so the
        // 1.5s window can't stretch under low timeScale, and the finally releases even if stopped.
        var rescued = _grabbedPlayer;          // capture — _grabbedPlayer may clear before release
        rescued?.Health?.SetInvincible(true);
        try { yield return new WaitForSecondsRealtime(1.5f); }
        finally { rescued?.Health?.SetInvincible(false); }
    }
