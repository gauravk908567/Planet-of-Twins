...
public class PlayerMovementController : MonoBehaviour, IMovementFreezable, ISlowReceiver
{
    ...
    // ── Slow (ISlowReceiver — bomb debuff etc.) ───────────────
    // Composes multiplicatively with _speedMultiplier. 1f = no slow.
    ...
    // ── ISlowReceiver (bomb debuff etc.) ──────────────────────
    /// <summary>Sustained movement slow (e.g. Witness/Siphon bomb, ×0.65 for 1.5 s). Composes with the Vigil
    /// speed multiplier and restores to full when it ends. Re-applying restarts the timer.</summary>
    ...
    private IEnumerator SlowRoutine(float multiplier, float duration)
    {
        ...
        yield return new WaitForSeconds(duration);   // scaled — debuff lasts N game-seconds (matches Enemy.SlowRoutine)
        _slowMultiplier = 1f;
        _slowCoroutine = null;
    }
    ...
    public void ExecuteCommand(IPlayerCommand command)
    {
        ...
        if (!_soulMode)
        {
            ...
            // _speedMultiplier (Vigil) and _slowMultiplier (bomb slow) both compound with base moveSpeed.
            // Both default to 1f so normal play is unchanged.
            Vector3 finalMove = move * (moveSpeed * _speedMultiplier * _slowMultiplier) * moveDt;
