...
public class EmpowerSystem : MonoBehaviour, IAbilityActiveState, IAbilityHUDSource
{
    ...
    [Header("VFX")]
    [Tooltip("Particle prefab played on anchoring twin each knockback pulse. Looping OFF — duration matches pulse interval.")]
    [SerializeField] private GameObject _knockbackParticlePrefab;
    [Tooltip("Local position offset for particle spawn on the anchoring twin.")]
    ...
    private ParticleSystem _knockbackParticleInstance;
    ...
    private void SpawnKnockbackParticle()
    {
        if (_knockbackParticlePrefab == null || _anchoringTwin == null) return;

        var go = Instantiate(_knockbackParticlePrefab,
                             _anchoringTwin.transform.position + _anchoringTwin.transform.TransformDirection(_particleSpawnOffset),
                             _knockbackParticlePrefab.transform.rotation,
                             _anchoringTwin.transform);

        _knockbackParticleInstance = go.GetComponent<ParticleSystem>();

        if (_knockbackParticleInstance != null)
        {
            // Stop first (handles Play On Awake), then set duration, then clear
            _knockbackParticleInstance.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
            var main = _knockbackParticleInstance.main;
            main.duration = KnockbackInterval;
        }
    }

    private void PlayKnockbackParticle()
    {
        if (_knockbackParticleInstance == null) return;
        _knockbackParticleInstance.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
        _knockbackParticleInstance.Play(true);
    }

    private void DestroyKnockbackParticle()
    {
        if (_knockbackParticleInstance == null) return;
        Destroy(_knockbackParticleInstance.gameObject);
        _knockbackParticleInstance = null;
    }
