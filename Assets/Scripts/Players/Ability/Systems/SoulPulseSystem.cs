...
public class SoulPulseSystem : MonoBehaviour
{
    ...
    [Header("VFX")]
    // Book pulled from PlayerVfxLibrary.SoulPulse via VfxLibraryProvider (R4, resolved lazily at play time).
    private CueBookData _cueBook;
    [Tooltip("Optional explicit FxManager (R1, Persistent→Persistent). Resolves to .Instance otherwise (R4).")]
    ...
    {
        // Pulse VFX at soul position — pooled cue book ("pulse_fire"), no per-call Instantiate/Destroy.
        _fx ??= FxManager.Instance;                                   // R4
        _cueBook ??= VfxLibraryProvider.Instance?.Player?.SoulPulse;  // R4 — book from PlayerVfxLibrary
        if (_cueBook != null)
        {
            // Follows the soul (this system lives on the SoulPlayer). emitterScale matches _pulseRadius (base 4, fixed).
 // Tier-resolved: plays pulse_fire_t[n] when authored in the book, else the base id.
            _fx?.PlayBook(_cueBook,
                UpgradeCueResolver.Resolve(_cueBook, _gateData, FxIds.Player.SoulPulse.pulse_fire),
                CueContext.Follow(transform, emitterScale: _pulseRadius / 4f, simSpeed: 1.5f));
        }
