...
public class CoalesceSystem : MonoBehaviour
{
    ...
    void HandleApplied(GameObject enemy)
    {
        ...
        // P16: pooled — spawn at the enemy, then parent so the aura rides it (the pool reparents
        // back to its category root on despawn, so a pooled ENEMY can never drag the aura along).
        var go = GameplayPool.Spawn(_auraPrefab, PoolCategory.AbilityObjects, enemy.transform.position, Quaternion.identity);
        var aura = go != null ? go.GetComponent<CoalesceAura>() : null;
        if (aura == null) { Debug.LogError("[Coalesce] CoalesceAura component missing from prefab"); GameplayPool.Despawn(go); return; }
        go.transform.SetParent(enemy.transform, worldPositionStays: true);
