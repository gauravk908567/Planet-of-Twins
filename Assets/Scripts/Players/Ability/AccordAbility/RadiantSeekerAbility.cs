...
public class RadiantSeekerAbility : IAbility, IAbilityHUDSource, IAbilityActiveState
{
    ...
    // "radorb_cast" = cast burst at spawn. Book resolved lazily from PlayerVfxLibrary (R4).
    ...
    public void TryActivate()
    {
        ...
        var go = GameplayPool.Spawn(_orbPrefab, PoolCategory.AbilityObjects, spawnPosition, Quaternion.identity);
        var orb = go != null ? go.GetComponent<RadiantSeekerOrb>() : null;
        ...
        // Cast burst at the spawn point (World). The orb's travel body is its prefab; hit/hiteffect fire on Detonate.
        _cueBook ??= VfxLibraryProvider.Instance?.Player?.RadiantSeeker;   // R4
        if (_cueBook != null)
            FxManager.Instance?.PlayBook(_cueBook, FxIds.Player.RadiantSeeker.radorb_cast, new CueContext(spawnPosition));
    }
}