...
public class VoidStrikeAbility : IAbility, IAbilityHUDSource, IAbilityActiveState
{
    ...
    // on_voidstrikecast = held hazard-point void; on_voidstrikeTakingDamage = per-enemy damage reaction on each tick.
    // Pulled from PlayerVfxLibrary.VoidStrike (R4, lazy).
    ...
    private IEnumerator RunAbility()
    {
        ...
        var activeVFX = new Queue<CueHandle>();
        ...
        while (elapsed < _duration)
        {
            ...
            if (elapsed >= nextSpawn)
            {
                ...
                if (activePoints.Count >= _maxPoints)
                {
                    ...
                    FxManager.Instance?.Stop(activeVFX.Dequeue());
                }

                activePoints.Enqueue(point);

                // Spawn the hazard-point cue (held until removed) — the void that falls and lingers. Author the
                // element prefab's shape radius to match _pointRadius — the cue system doesn't tweak per-instance.
                var handle = _cueBook != null
                    ? (FxManager.Instance?.PlayBook(_cueBook, FxIds.Player.VoidStrike.on_voidstrikecast,
                        new CueContext(point)) ?? CueHandle.None)
                    : CueHandle.None;
                activeVFX.Enqueue(handle);
            ...
            if (elapsed >= nextTick)
            {
                foreach (Vector3 pt in activePoints)
                {
                    ...
                    {
                        var hp = hit.GetComponent<EnemyHealthComponent>();
                        if (hp == null) continue;
                        hp.TakeDamage(new DamageData(_damagePerTick, DamageType.Ability));

                        // Damage-taken reaction on each enemy standing in the void — World-anchored at chest height.
                        if (_cueBook != null)
                            FxManager.Instance?.PlayBook(_cueBook, FxIds.Player.VoidStrike.on_voidstrikeTakingDamage,
        ...
        // Cleanup all remaining point cues
        foreach (var handle in activeVFX)
