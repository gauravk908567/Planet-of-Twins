...
public class AbilityUpgradeNode
{
 ...
 // ── Tier VFX — NO field needed: tier visuals are the `_t[n]` naming convention ──
    // At tier N (= unlocked node count) every cue id the ability plays tries `<id>_tN` … `_t1`
    // in its book, else the base id (UpgradeCueResolver). Author `<id>_t2` in the CueBookData
    // to change that one effect at tier 2; ids without `_t[n]` variants serve ALL tiers.
